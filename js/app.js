...
***************************************************************************** */
        ...
        function documentActions(e) {
            ...
            if (targetElement.classList.contains("search-form__icon")) document.querySelector(".search-form").classList.toggle("_active"); else if (!targetElement.closest(".search-form") && document.querySelectorAll(".search-form._active")) document.querySelector(".search-form").classList.remove("_active");
        ...
        async function getProducts(button) {
            if (!button.classList.contains("_hold")) {
                ...
                if (response.ok) {
                    ...
                    button.remove();
                } else alert("При загрузке произошла ошибка");
            }
        }
        ...
        function addToCart(productButton, productId) {
            ...
            const transitionend = document.querySelector("._flyimage");
            transitionend.addEventListener("transitionend", (function() {
                let productImageFly = document.querySelector("._flyimage");
        ...
        const furtiture = document.querySelector(".furtiture__body");
        if (furtiture && !isMobile.any()) {
            const furtitureItems = document.querySelector(".furtiture__items");
            const furtitureColumn = document.querySelectorAll(".furtiture__column");
            const speed = furtiture.dataset.speed;
            let positionX = 0;
            let coordXprocent = 0;
            function setMouseGalleryStyle() {
                let furtitureItemsWidth = 0;
                furtitureColumn.forEach((element => {
                    furtitureItemsWidth += element.offsetWidth;
                }));
                const furtitureDifferent = furtitureItemsWidth - furtiture.offsetWidth;
                const distX = Math.floor(coordXprocent - positionX);
                positionX += distX * speed;
                let position = furtitureDifferent / 200 * positionX;
                furtitureItems.style.cssText = `transform: translate3d(${-position}px,0,0)`;
                if (Math.abs(distX) > 0) requestAnimationFrame(setMouseGalleryStyle); else furtiture.classList.remove("_init");
            }
            furtiture.addEventListener("mousemove", (function(e) {
                const furtitureWidth = furtiture.offsetWidth;
                const coordX = e.pageX - furtitureWidth / 2;
                coordXprocent = coordX / furtitureWidth * 200;
                if (!furtiture.classList.contains("_init")) {
                    requestAnimationFrame(setMouseGalleryStyle);
                    furtiture.classList.add("_init");
                }
            }));
        }
        isWebp();
